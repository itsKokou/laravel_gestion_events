    ...
    <form method="POST" action="{{ route('public.reservations.store', $event) }}" class="card">
        ...
        <div class="grid grid2" style="margin-top: 12px;">
                ...
                <select id="ticket_type_id" name="ticket_type_id" required>
                    ...
                    @foreach ($event->ticketTypes as $type)
                        <option value="{{ $type->id }}" @selected((string) old('ticket_type_id') === (string) $type->id)>
                            {{ $type->name }} — {{ number_format($type->price_cents / 100, 2, ',', ' ') }} {{ $type->currency }}
                        </option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <input id="quantity" name="quantity" type="number" min="1" max="10" value="{{ old('quantity', 1) }}" required />
                <div class="muted" style="font-size: 12px; margin-top: 6px;">Vous devrez remplir les infos de chaque participant.</div>
            </div>
        </div>
                    ...
                    @foreach ($event->addons as $addon)
                        <label class="card" style="padding: 12px; display:flex; gap:10px; align-items:flex-start;">
                            ...
                            <div style="flex:1;">
                                ...
                                <div class="muted" style="font-size: 13px;">{{ number_format($addon->price_cents / 100, 2, ',', ' ') }} {{ $addon->currency }}</div>
                            </div>
                        </label>
                    @endforeach
                </div>
        ...
        <div style="margin-top: 12px;">
            <label style="display:flex; gap:10px; align-items:flex-start;">
                <input type="checkbox" name="agree_terms" value="1" style="width:auto; margin-top: 4px;" required @checked(old('agree_terms')) />
