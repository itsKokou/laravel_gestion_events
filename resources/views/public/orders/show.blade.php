    ...
    <div style="display:flex; justify-content:space-between; gap:12px; flex-wrap:wrap;">
        <div>
            ...
            <div class="muted" style="margin-top: 6px;">
                Total : <strong>{{ number_format($order->total_cents, 0, ',', ' ') }}
        ...
        <div style="display:flex; gap:10px;">
            ...
            @if ($order->status === 'paid')
                <a class="btn secondary" href="{{ route('public.orders.invoice', $order) }}">Facture PDF</a>
            @endif
            @if ($order->status !== 'paid')
                ...
                <div style="margin-top: 10px; display:flex; gap:12px; align-items:center; flex-wrap:wrap;">
                    <div class="card"
                        ...
                        <img src="{{ route('tickets.qr', $ticket) }}" alt="QR Code"
                            style="width: 180px; height: 180px; background: #fff; border-radius: 12px; padding: 10px;" />
                    </div>
                    <div class="card" style="padding: 10px 12px; border-radius: 12px; flex:1;">
                        <div class="muted" style="font-size: 12px;">Token (support)</div>
                        <div
                            style="font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, 'Liberation Mono', 'Courier New', monospace; font-size: 12px; word-break: break-all;">
                            {{ $ticket->qr_token }}
                        </div>
                    </div>
                </div>
